...
out vec2 lodestone_xy;
out vec3 lodestone_topLeft;
out vec3 lodestone_pos;
out vec4 lodestone_position0;
out vec4 lodestone_position2;
out vec4 lodestone_position13;
out vec4 lodestone_color;
...
void border(int vertId) {
    lodestone_pos = Position;

    const vec2[4] corners = vec2[4](vec2(0), vec2(0, 1), vec2(1, 1), vec2(1, 0));
    lodestone_xy = corners[vertId];
    if (vertId == 0) {
        lodestone_topLeft = vec3(texCoord0, 1);
        lodestone_position0 = vec4(Position, 1);
    }
    if (vertId == 2) {
        lodestone_position2 = vec4(Position, 1);
    }
    if (vertId == 1 || vertId == 3) {
        lodestone_position13 = vec4(Position, 1);
    }
}
...
void main() {
    type = TYPE_VANILLA;
    lodestone_xy = vec2(0);
    lodestone_pos = lodestone_topLeft = vec3(0);
    lodestone_position13 = lodestone_position2 = lodestone_position0 = vec4(0);
    lodestone_color = Color;
